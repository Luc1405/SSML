...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Restrict training to these months"
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Include raw dayofyear in addition to doysin doycos for tree models"
    )
    ap.add_argument(
        "--include-latlon",
        action="store_true",
        help="Include lat lon"
    )
    args = ap.parse_args()
    ...
    if len(linear_feature_cols) == 0:
        raise ValueError("No valid linear-model features")
    if len(tree_feature_cols) == 0:
        raise ValueError("No valid tree model features")
    ...
    print("Validation spatial_cv_only_all_years")
    print("Calibration")
